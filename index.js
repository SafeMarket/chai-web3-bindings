...
module.exports = function chaiWeb3Bindings(chai, utils) {

  const Assertion = chai.Assertion

  Assertion.addMethod('hex', function addHexProperty() {
    this.assert(
      web3Utils.toHex(this._obj) === this._obj,
  ...
  Assertion.addMethod('contract', function addContractPropertyProperty() {
    this.assert(
      this._obj.address && web3Utils.isAddress(this._obj.address) && !isZeros(this._obj.address),
  ...
  Assertion.addMethod('ascii', function addContractPropertyProperty(thing) {
    ...
    const ascii = web3Utils.toHex(this._obj) === this._obj ?
      web3Utils.toAscii(this._obj).split(String.fromCharCode(0)).join('') : this._obj
